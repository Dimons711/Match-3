...
public class Match3 : MonoBehaviour
{
    ...
    {
        if (getValueAtPoint(one) < 0) return; // Cheking if there are a holes 
    ...
    List<Point> isConnected(Point p, bool main)
    {
        ...
        foreach (Point dir in directions) // Checking if there is 2 oe more same shapes in the directions
        {
            ...
            if (same > 1) // If there are more than 1 same shape in the directions then we know it is a match
                AddPoints(ref connected, line); // Add these points to the overeach connected list
        }
        for (int i = 0; i < 2; i++) //Checking if we are in the middle of the same shapes
        {
            ...
            foreach (Point next in check) //Check both sides of the piece, if they are the same value, add the to the list
            {
        ...
        for (int i = 0; i < 4; i++) //Check for a 2x2
        {
            ...
            foreach (Point pnt in check) //Check all sides of the piece, if they are the same value, add the to the list
            {
